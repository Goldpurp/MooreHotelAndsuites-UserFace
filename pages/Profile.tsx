...
const getLastName = (name: string) => {
  ...
  const validateSecurity = () => {
    ...
    if (securityData.newPassword !== securityData.confirmNewPassword) errors.confirmNewPassword = "Keys do not match.";
    ...
    <div className="pt-24 md:pt-32 pb-24 px-4 md:px-12 min-h-screen bg-background-dark relative overflow-hidden">
      ...
      <div className="max-w-[1200px] mx-auto">
        ...
        <header className="mb-16 md:mb-20 text-center space-y-6 animate-in fade-in duration-1000">
          <p className="text-primary text-[10px] sm:text-[11px] uppercase tracking-[0.8em] font-black">Private Registry</p>
        ...
        <main className="relative z-10 space-y-12">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 animate-in fade-in slide-in-from-bottom-8 duration-700">
              <div className="bg-surface-dark border border-white/5 p-8 md:p-12 hover:border-primary/20 transition-all group relative overflow-hidden">
                <div className="space-y-4">
                  <p className="text-[10px] uppercase tracking-[0.5em] text-gray-600 font-black">Profile Name</p>
                  <p className="text-2xl md:text-3xl text-white group-hover:text-primary transition-colors">  {capitalize(user.name)}</p>
                </div>
              </div>
              ...
              <div className="bg-surface-dark border border-white/5 p-8 md:p-12 hover:border-primary/20 transition-all group relative overflow-hidden">
                ...
                <div className="space-y-4">
                  ...
                  <p className="text-2xl md:text-3xl text-white group-hover:text-primary transition-colors">{user.email}</p>
                </div>
              </div>
              ...
              <div className="bg-surface-dark border border-white/5 px-8 py-4 md:px-12 md:py-8 hover:border-primary/20 transition-all group relative overflow-hidden">
                <div className="space-y-4">
                  <p className="text-[10px] uppercase tracking-[0.5em] text-gray-600 font-black">Joined On</p>
                  <p className="text-sm md:text-xl text-white group-hover:text-primary transition-colors">{user.createdAt ? formatDate(user.createdAt) : "—"}</p>
                </div>
              </div>
          ...
          {activeTab === "archive" && (
            <div className="space-y-6 animate-in fade-in slide-in-from-bottom-8 duration-700">
              {bookings.length > 0 ? (
                bookings.map((b) => (
                  <div key={b.id} className="bg-surface-dark border border-white/5 p-6 md:p-10 flex flex-col md:flex-row md:items-center justify-between gap-6 md:gap-8 group hover:border-primary/20 transition-all">
                    <div className="space-y-1">
                      <p className="text-[9px] text-primary uppercase tracking-[0.5em] font-black">Stay Record</p>
                      <h4 className="serif-font text-2xl md:text-3xl text-white group-hover:text-primary transition-colors">{b.bookingCode}</h4>
                    </div>
                    
                    <div className="flex flex-wrap gap-6 md:gap-12">
                      <div className="space-y-1">
                        <p className="text-[8px] text-gray-600 uppercase tracking-widest font-black">Check-in</p>
                        <p className="text-white text-base md:text-lg font-bold">{b.checkIn ? new Date(b.checkIn).toLocaleDateString() : "—"}</p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-[8px] text-gray-600 uppercase tracking-widest font-black">Check-in</p>
                        <p className="text-white text-base md:text-lg font-bold">{b.checkIn ? new Date(b.checkOut).toLocaleDateString() : "—"}</p>
                      </div>
                      <div className="space-y-1">
                        <p className="text-[8px] text-gray-600 uppercase tracking-widest font-black">Status</p>
                        <p className="text-primary text-[12px] font-black uppercase tracking-widest">{b.status}</p>
                      </div>
                    </div>
                  </div>
                ))
                ...
                <div className="py-32 md:py-40 text-center border border-white/5 border-dashed rounded-sm bg-white/[0.01]">
                  <p className="serif-font text-2xl md:text-3xl text-gray-700">No historical records found in registry.</p>
                </div>
              )}
            </div>
          )}
          ...
          {activeTab === "security" && (
            <div className="animate-in fade-in slide-in-from-bottom-8 duration-700 max-w-2xl mx-auto">
              <form onSubmit={handleRotateSecurity} className="bg-surface-dark border border-white/5 p-8 md:p-16 space-y-8 md:space-y-10 shadow-2xl relative overflow-hidden">
                <div className="space-y-2">
                  <h3 className="serif-font text-3xl md:text-4xl text-white">Rotate Security Key</h3>
                  <p className="text-gray-500 text-[9px] sm:text-[10px] uppercase tracking-widest font-black opacity-60">Maintain vault integrity through regular rotation.</p>
                </div>
                  ...
                  {["oldPassword", "newPassword", "confirmNewPassword"].map((field, idx) => (
                    <div key={idx} className="space-y-1">
                      <label className="text-[9px] text-gray-600 font-black uppercase tracking-[0.3em] ml-1">
                        {field === "oldPassword" ? "Current Key" : field === "newPassword" ? "New Key" : "Confirm New Key"}
                      </label>
                      <input
                        required
                        type="password"
                        value={securityData[field as keyof typeof securityData]}
                      ...
                      />
                      {fieldErrors[field] && <p className="text-red-500 text-[8px] uppercase font-black tracking-widest ml-1">{fieldErrors[field]}</p>}
                    </div>
                  ))}
                </div>
