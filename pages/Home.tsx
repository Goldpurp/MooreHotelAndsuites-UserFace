...
const Home: React.FC = () => {
  ...
  const handleSearch = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      ...
      if (searchData.category && searchData.category !== "All") params.append("category", searchData.category);
  ...
  const handleSubscribe = (e: React.FormEvent) => {
    ...
    setTimeout(() => {
      setModal({
        ...
        message: 'Welcome to the Moore Circle. Your request is being reviewed by our membership board.',
        type: 'success'
      });
    ...
    <div className="bg-background-dark min-h-screen">
              ...
              <div className="bg-black/40 backdrop-blur-3xl border border-white/10 p-1 rounded-sm flex flex-col md:flex-row items-stretch gap-1 shadow-2xl">
                <div className="flex-1 grid grid-cols-2 lg:grid-cols-4 divide-x divide-white/5">
                  <div className="px-4 py-3 flex flex-col items-start gap-1">
                    <label className="text-[8px] uppercase tracking-[0.2em] text-gray-500 font-black">Room Tier</label>
                    <select
                    ...
                    >
                      <option className="bg-black" value="All">All Suites</option>
                      {Object.values(RoomCategory).map((cat) => (
                        <option key={cat} className="bg-black" value={cat}>{cat} Tier</option>
                      ))}
                    </select>
                  </div>
                  <div className="px-4 py-3 flex flex-col items-start gap-1 bg-white/[0.07] border-x border-primary/40 group focus-within:ring-2 ring-primary/60 transition-all shadow-inner">
                    <label className="text-[8px] uppercase tracking-[0.2em] text-primary font-black">Check-in</label>
                    <input
                  ...
                  <div className="px-4 py-3 flex flex-col items-start gap-1 bg-white/[0.07] border-primary/40 group focus-within:ring-2 ring-primary/60 transition-all shadow-inner">
                    <label className="text-[8px] uppercase tracking-[0.2em] text-primary font-black">Check-out</label>
                    <input
                      className="bg-transparent border-none p-0 text-xs focus:ring-0 w-full text-white font-bold cursor-pointer outline-none"
                      type="date"
                  ...
                  <div className="px-4 py-3 flex flex-col items-start gap-1">
                    <label className="text-[8px] uppercase tracking-[0.2em] text-gray-500 font-black">Guests</label>
                    <select
                    ...
                    >
                      {[1, 2, 3, 4].map((num) => (
                        <option key={num} className="bg-black" value={num}>{num} {num > 1 ? "Guests" : "Guest"}</option>
                      ))}
                    </select>
                  </div>
                </div>
                ...
                >
                  {searching && <div className="w-3 h-3 border-2 border-black/30 border-t-black rounded-full animate-spin"></div>}
      ...
      <section className="py-24 md:py-40 px-6 overflow-hidden">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-16 md:gap-24">
          <div className="flex-1 space-y-10 order-2 lg:order-1">
            <div className="space-y-4">
              <p className="text-primary text-[10px] font-black uppercase tracking-[0.5em]">Nigerian Professionalism</p>
              <h2 className="serif-font text-4xl md:text-7xl text-white italic leading-tight">
                Refined Hotel Service,
                <br />At Every Touchpoint.
              </h2>
            </div>
        ...
        <div className="max-w-[1800px] mx-auto">
          <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
            <div className="space-y-4 text-left">
              <p className="text-primary text-[10px] font-black uppercase tracking-[0.5em]">Executive Room Collection</p>
              <h2 className="serif-font text-6xl md:text-8xl text-white italic leading-none">The Registry</h2>
            </div>
            <Link to="/rooms" className="text-[10px] font-black uppercase tracking-[0.4em] text-gray-500 hover:text-primary transition-all flex items-center gap-4">
              View All Accommodations <span className="material-symbols-outlined text-lg">arrow_forward</span>
            </Link>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 md:gap-12">
            {loading
              ? [1, 2, 3, 4].map((i) => <div key={i} className="aspect-[3/4] bg-white/[0.02] animate-pulse rounded-sm" />)
              : featuredRooms.map((room) => (
                  <Link key={room.id} to={`/rooms/${room.id}?${getSearchQuery()}`} className="group block space-y-8">
                    <div className="relative aspect-[3/4] overflow-hidden rounded-sm border border-white/5 shadow-[0_20px_40px_rgba(0,0,0,0.6)] transition-all duration-[1000ms] group-hover:-translate-y-2">
                      <img src={room.images?.[0]} className="w-full h-full object-cover grayscale-[0.3] group-hover:grayscale-0 transition-transform duration-[5000ms] group-hover:scale-110" alt={room.name} />
                      <div className="absolute inset-0 bg-gradient-to-t from-black via-transparent to-transparent opacity-90" />
                      <div className="absolute bottom-8 left-8 right-8 flex flex-col gap-1">
                        <p className="text-primary text-[8px] font-black uppercase tracking-[0.4em]">{room.category} Tier</p>
                        <h3 className="serif-font text-3xl text-white italic">{room.name}</h3>
                      </div>
                    </div>
                  </Link>
                ))}
          ...
          <div className="grid lg:grid-cols-12 gap-16 items-center">
            <div className="lg:col-span-5 space-y-10">
              <div className="space-y-5">
                <p className="text-primary text-[10px] font-black uppercase tracking-[0.5em]">Hotel Perspective</p>
                <h2 className="serif-font text-5xl md:text-7xl text-white italic leading-[1.1]">The Moore Experience</h2>
              </div>
              <p className="text-gray-400 text-lg md:text-xl font-light leading-relaxed max-w-xl">
                Experience the transition from Lagos' energetic business streets to the
                calm, professional luxury of Moore Hotels.
              </p>
              <button className="flex items-center gap-6 group">
                ...
                <span className="text-[10px] font-black uppercase tracking-[0.4em] text-white">Watch Anthem</span>
              </button>
            </div>
      ...
      <section className="py-32 md:py-48 bg-black text-center px-6 border-y border-white/5">
        <div className="max-w-4xl mx-auto space-y-12">
          ...
          <p className="text-[11px] uppercase tracking-[0.4em] text-primary font-black">Ademola Moore</p>
        </div>
      </section>
      ...
      <section className="py-32 md:py-48 px-6 text-center">
        <div className="max-w-4xl mx-auto space-y-12">
          <h2 className="serif-font text-5xl md:text-9xl text-white italic leading-none">Join Moore Circle.</h2>
          <p className="text-gray-500 text-lg md:text-xl font-light">Receive exclusive invitations, corporate rates, and priority guest services.</p>
            ...
            <form onSubmit={handleSubscribe} className="flex flex-col gap-4">
              <div className="space-y-2">
                <input 
                  ...
                  className={`w-full bg-white/[0.03] border ${emailError ? 'border-red-500/50' : 'border-white/10'} p-5 text-white outline-none focus:border-primary transition-all font-light italic`}
                />
                {emailError && <p className="text-red-500 text-[8px] uppercase font-black tracking-widest text-left ml-1">{emailError}</p>}
              </div>
              <button
                type="submit"
              ...
              >
                {subscribing && <div className="w-3 h-3 border-2 border-black/30 border-t-black rounded-full animate-spin"></div>}
